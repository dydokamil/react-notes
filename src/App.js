  ...
  componentWillReceiveProps = (props) => {
    if (
      ...
      if ('content' in props.notes.notes[props.notes.selected]) {
        this.setState({ content: props.notes.notes[props.notes.selected].content });
      } else {
          ...
          <div className="left">
            ...
            {_.map(_.orderBy(this.props.notes.notes, 'edited', 'desc'), note => (
              <div
                ...
                key={note._id}
                className={`note ${
                  note._id === this.props.notes.selected ? 'note-selected' : 'note-hoverable'
                }`}
              >
                <div className="note-name">
                  {note.name ? (
                    ...
                    <span style={{ fontStyle: 'italic', fontWeight: 100 }}>no title</span>
                  )}
                </div>
                <div className="note-date-preview">{formatDatetime(note.edited)}</div>
              </div>
            ))}
          </div>
          <div className="right">
